...
void setMultiDevices(){
  fauxmo.addDevice(ALEXA_COMMAND_ONE);
  fauxmo.addDevice(ALEXA_COMMAND_TWO);
}
...
void loadCredentials() {
  ...
  String recipe7 = readFile(LittleFS, "/alexaone.txt");
  if (!recipe7.equals("none")){
    strcpy(ALEXA_COMMAND_ONE,recipe7.c_str());
  }

  String recipe8 = readFile(LittleFS, "/alexatwo.txt");
  if (!recipe8.equals("none")){
...
void setValuesToVars(uint8_t target, char *content) {
  ...
  else if (target == IS_ALEXA_ONE) {
    writeFile(LittleFS, "/alexaone.txt", content);
  }
  else if (target == IS_ALEXA_TWO) {
    writeFile(LittleFS, "/alexatwo.txt", content);
  }
}
...
void serverSetup() {
  ...
  */
  ...
  [](AsyncWebServerRequest * request) {
    ...
    response->print("IDENTIFICADOR 1: ");
    response->print(String(ALEXA_COMMAND_ONE));
    response->print("<br>");

    response->print("IDENTIFICADOR 2: ");
    response->print(String(ALEXA_COMMAND_TWO));
    response->print("<br>");
